...
bool fcpu_dec;
struct timespec cpu_dec_time = {0,0};
...
static char const short_options[] =
	...
	"S"
#endif
	"a:c:Dhp:Px:qr:R:s:t:e:T:o:u:O:V";
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		struct timeval tv_start, tv_end, diff;
		struct timeval ttest_start, ttest_end, tdiff;
		int64_t max64;
		int i_, rc;
		double dt_0, dt_m, dt_1;
		double hashrate_t, hashrate_t2, tol;
		...
		{
		      	fhash_test = false;	
			if(opt_cpu_eff == 100) { fcpu_dec = false; }
			else {
				fcpu_dec = true;
				...
				dt_0 = 0.;
				dt_1 = (99. * 10. * 1.e9 / hashrate_t);

				tol = 0.03;
				if (opt_cpu_eff <= 5) tol = 0.01;

				for (i_=1; i_<=100; i_++)
				{
					dt_m = (dt_0 + dt_1) / 2.;
					cpu_dec_time.tv_sec  = (int64_t)dt_m/(int64_t)1e9;
					...
					if (abs(hashrate_t2 - hashrate_t * opt_cpu_eff / 100.) < hashrate_t * opt_cpu_eff / 100. * tol) break;
					if (hashrate_t2 > hashrate_t * opt_cpu_eff / 100.) {
						dt_0 = dt_m;
					}
					else {
						dt_1 = dt_m;
				...
				applog(LOG_INFO, "hash-test (1 thread; %d tries): %d hashes, %d hash/s (dec: %d us)", i_, hashes_done, (int64_t)(hashrate_t), (int64_t)(cpu_dec_time.tv_sec*1e6+cpu_dec_time.tv_nsec*1e-3));
			}
		}
